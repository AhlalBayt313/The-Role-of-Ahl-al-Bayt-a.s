...
//
// SECTION 2 APPROACH:
//   The "ইমাম ও মাসুম (আ)" tab and the "বংশবৃক্ষ" tab do NOT reimplement any
...
function renderAhlulBaytCategoryChips() {
    ...
    const CATEGORIES = [
        { key: 'masumeen',  icon: '👑', bn: 'ইমাম ও মাসুম (আ)',      en: '14 Masumeen (AS)' },
...
function getAhlulBaytSearchIndex() {
    ...
    // Bug fix (2026-09-03): extras that now carry a full detail page (Zaynab,
    // Abbas — flagged by the presence of birthBn, see script-3-pages.js's
    // renderCard/renderImamDetailPage) previously had no way to reach that
    // page from HERE (search box selection, or the Male/Female Personalities
    // tabs' renderPersonalityCard — both funnel through this index +
    // ahlulBaytSelectSearchResult → the side panel below). detailId stays
    // null (no showPersonDetail() family-tree modal exists for these, still
    // true) — hasFullPage is a separate new flag the side panel checks.
    ADDITIONAL_PERSONALITIES.forEach(p => idx.push({
        source: 'extra', id: p.id, detailId: null, hasFullPage: !!p.birthBn,
        nameBn: p.nameBn, nameEn: p.nameEn, epithetBn: p.epithetBn, epithetEn: p.epithetEn,
        descBn: p.descBn, descEn: p.descEn, arabicName: p.arabicName, icon: p.icon
    }));
    return idx;
...
function renderAhlulBaytSidePanel() {
    ...
    const viewBtn = person.detailId != null ? `
        ...
        </button>` : (person.source === 'extra' && person.hasFullPage) ? `
        <button type="button" data-action="viewImam" data-param="${person.id}"
            style="margin-top:.9rem;font-size:.78rem;font-weight:700;padding:8px 16px;border-radius:999px;cursor:pointer;
                border:1px solid ${d?'rgba(180,83,9,.4)':'rgba(180,83,9,.3)'};
                background:${d?'rgba(180,83,9,.15)':'rgba(180,83,9,.08)'};color:${d?'#fcd34d':'#b45309'}">
            ${l==='bn'?'সম্পূর্ণ প্রোফাইল দেখুন':'View Full Profile'}
...
// ============================================================================
// IMAM 3D COVERFLOW CAROUSEL (added 2026-09-07)
// ----------------------------------------------------------------------------
// Outer presentation/position layer for the existing 12-Imam card grid
// (renderImamsPage(), script-3-pages.js). Positions/scales/rotates the
// unchanged renderCard() output via CSS custom properties + classes only —
// never touches card-internal markup, imamFlip(), imamCardParticles(), or
// the Details/Share buttons above. render() (script-4-boot.js) fully
// rebuilds the DOM on every call (dark mode, language, timeline toggle,
// etc.), so the current card index lives here in a module-level variable
// (not in `state`, which is persisted/serialized elsewhere) and is simply
// re-read the next time renderImamsPage() runs — the freshly rendered
// markup is generated with the correct classes already applied, so no
// separate "re-init after render" step is needed anywhere.
// ============================================================================
// NOTE: this whole file's body runs inside the installAhlulBaytFallback()
// IIFE (see below), so a plain top-level `var` here would stay trapped
// inside that closure and be invisible to script-3-pages.js. `_imamCoverflowIndex`
// is therefore kept directly on the global object from the start; the
// helper functions below are exported the same way the file's own
// pre-existing functions are, in the "Install onto the global scope"
// block further down.
var g = (typeof window !== 'undefined') ? window : globalThis;
if (typeof g._imamCoverflowIndex !== 'number') g._imamCoverflowIndex = 0;

/** Shortest-path signed distance from `activeIndex` to `idx`, wrapping
 *  around `count` (so with 12 imams, index 11 is offset -1 from index 0,
 *  not +11). Shared by both the initial server-rendered markup
 *  (renderImamsPage()) and the runtime DOM updater below, so the two can
 *  never drift out of sync with each other. */
function _imamCoverflowOffset(idx, activeIndex, count) {
    if (!count) return 0;
...
/** Which visual band (center / immediate neighbor / far / hidden) a given
 *  offset falls into — mirrors the .imam-coverflow-card.is-* rules in
 *  style.css. */
function _imamCoverflowTier(offset) {
    const abs = Math.abs(offset);
    if (abs === 0) return 'is-active';
    if (abs === 1) return offset < 0 ? 'is-prev' : 'is-next';
    if (abs === 2) return 'is-far';
    return 'is-hidden';
}
function _imamCoverflowClamp(i, count) {
    if (count <= 0) return 0;
    return ((i % count) + count) % count;
}

/** Re-applies is-active/is-prev/is-next/is-far/is-hidden + --cf-offset to
 *  every card currently in the DOM, based on _imamCoverflowIndex. Pure CSS
 *  transform/opacity update — no DOM rebuild, no render() call. Silently
 *  no-ops if the coverflow isn't currently on screen (any other page). */
function imamCoverflowApply() {
    const track = document.querySelector('[data-imam-coverflow-track]');
    if (!track) return;
    const cards = track.children;
    const count = cards.length;
    if (!count) return;
    g._imamCoverflowIndex = _imamCoverflowClamp(g._imamCoverflowIndex, count);

    for (let i = 0; i < count; i++) {
        const card = cards[i];
        const offset = _imamCoverflowOffset(i, g._imamCoverflowIndex, count);
        const tier = _imamCoverflowTier(offset);
        card.classList.remove('is-active', 'is-prev', 'is-next', 'is-far', 'is-hidden');
        card.classList.add(tier);
        card.style.setProperty('--cf-offset', offset);
        card.setAttribute('aria-current', offset === 0 ? 'true' : 'false');
        card.setAttribute('tabindex', offset === 0 ? '0' : '-1');
    }

    document.querySelectorAll('[data-imam-coverflow-dot]').forEach(dot => {
        const isActive = parseInt(dot.getAttribute('data-imam-coverflow-dot'), 10) === g._imamCoverflowIndex;
        dot.classList.toggle('is-active', isActive);
        dot.setAttribute('aria-current', isActive ? 'true' : 'false');
    });
}

function imamCoverflowGoTo(index) {
    const track = document.querySelector('[data-imam-coverflow-track]');
    const count = track ? track.children.length : (typeof imams !== 'undefined' ? imams.length : 0);
    if (!count) return;
    g._imamCoverflowIndex = _imamCoverflowClamp(index, count);
    imamCoverflowApply();
}

function imamCoverflowNav(delta) {
    imamCoverflowGoTo(g._imamCoverflowIndex + Number(delta));
}

// ── মাসুমিন (Prophet & Masumeen) coverflow — same visual design as the
//    Imam coverflow above (identical .imam-coverflow-* CSS classes reused
//    as-is), and the same pure _imamCoverflowOffset/_imamCoverflowTier/
//    _imamCoverflowClamp helpers (they take no shared state, so nothing to
//    duplicate there). Kept as a fully separate index + set of
//    data-masumeen-* attributes/functions so the two carousels never
//    collide via the same document.querySelector() call — mirrors how
//    this file already keeps MACS/MACS2/MCONIC separate from ACS/ACS2/CONIC
//    for these two sections. ──
if (typeof g._masumeenCoverflowIndex !== 'number') g._masumeenCoverflowIndex = 0;

function masumeenCoverflowApply() {
    const track = document.querySelector('[data-masumeen-coverflow-track]');
    if (!track) return;
    const cards = track.children;
    const count = cards.length;
    if (!count) return;
    g._masumeenCoverflowIndex = _imamCoverflowClamp(g._masumeenCoverflowIndex, count);

    for (let i = 0; i < count; i++) {
        const card = cards[i];
        const offset = _imamCoverflowOffset(i, g._masumeenCoverflowIndex, count);
        const tier = _imamCoverflowTier(offset);
        card.classList.remove('is-active', 'is-prev', 'is-next', 'is-far', 'is-hidden');
        card.classList.add(tier);
        card.style.setProperty('--cf-offset', offset);
    ...
    document.querySelectorAll('[data-masumeen-coverflow-dot]').forEach(dot => {
        const isActive = parseInt(dot.getAttribute('data-masumeen-coverflow-dot'), 10) === g._masumeenCoverflowIndex;
        dot.classList.toggle('is-active', isActive);
        dot.setAttribute('aria-current', isActive ? 'true' : 'false');
    });
}

function masumeenCoverflowGoTo(index) {
    const track = document.querySelector('[data-masumeen-coverflow-track]');
    const count = track ? track.children.length : 0;
    if (!count) return;
    g._masumeenCoverflowIndex = _imamCoverflowClamp(index, count);
    masumeenCoverflowApply();
}

function masumeenCoverflowNav(delta) {
    masumeenCoverflowGoTo(g._masumeenCoverflowIndex + Number(delta));
}

// ── Click delegation (both carousels): nav buttons, dots, and "click a
//    side card to bring it to center" — registered ONCE at script load
//    (not per-render; this listener lives on `document` and survives every
//    render() DOM rebuild). Clicks on the existing Details/Share buttons
//    (which carry their own data-action, handled entirely by
//    script-2-ui.js's dispatcher) are explicitly left alone here so their
//    behavior is completely untouched. ──
document.addEventListener('click', function (e) {
    // Gate on whichever section wraps the click (nav buttons + dots are
    // siblings of the coverflow region itself, not descendants of it).
    const section = e.target.closest('[data-imam-coverflow-section], [data-masumeen-coverflow-section]');
    if (!section) return;
    const isM = section.hasAttribute('data-masumeen-coverflow-section');
    const nav      = isM ? masumeenCoverflowNav  : imamCoverflowNav;
    const goTo     = isM ? masumeenCoverflowGoTo : imamCoverflowGoTo;
    const navAttr  = isM ? 'data-masumeen-coverflow-nav'  : 'data-imam-coverflow-nav';
    const dotAttr  = isM ? 'data-masumeen-coverflow-dot'  : 'data-imam-coverflow-dot';
    const cardAttr = isM ? 'data-masumeen-coverflow-card' : 'data-coverflow-card';
    const idxAttr  = isM ? 'data-masumeen-index'          : 'data-imam-index';

    const navBtn = e.target.closest('[' + navAttr + ']');
    if (navBtn) { nav(parseInt(navBtn.getAttribute(navAttr), 10)); return; }

    const dot = e.target.closest('[' + dotAttr + ']');
    if (dot) { goTo(parseInt(dot.getAttribute(dotAttr), 10)); return; }

    // Anything inside an existing interactive control (Details/Share/the
    // flip card's "Back" button) keeps its own behavior — don't also
    // recenter the card underneath it.
    if (e.target.closest('button, a')) return;

    const card = e.target.closest('[' + cardAttr + ']');
    if (card) {
        const idx = parseInt(card.getAttribute(idxAttr), 10);
        if (!Number.isNaN(idx)) goTo(idx);
    }
});

// ── Keyboard (both carousels): ArrowLeft/ArrowRight/Home/End, only while
//    that specific carousel (or something inside it) has focus. ──
document.addEventListener('keydown', function (e) {
    const active = document.activeElement;
    const imamCf = active && active.closest && active.closest('[data-imam-coverflow]');
    const masCf  = !imamCf && active && active.closest && active.closest('[data-masumeen-coverflow]');
    if (!imamCf && !masCf) return;
    const nav       = masCf ? masumeenCoverflowNav  : imamCoverflowNav;
    const goTo      = masCf ? masumeenCoverflowGoTo : imamCoverflowGoTo;
    const trackSel  = masCf ? '[data-masumeen-coverflow-track]' : '[data-imam-coverflow-track]';
    if (e.key === 'ArrowLeft')       { nav(-1); e.preventDefault(); }
        ...
        if (track) goTo(track.children.length - 1);
        e.preventDefault();
    }
});

// ── Pointer drag / touch swipe (both carousels, delegated, attached
//    once). Suppresses the synthetic click that follows a real drag so a
//    swipe doesn't ALSO "select" whichever card happened to end up under
//    the pointer on release. ──
(function () {
    let dragState = null;
    let suppressNextClick = false;
    const SWIPE_THRESHOLD = 60; // px — within the spec's 50–70px range
    const TRACK_SEL = '[data-imam-coverflow-track], [data-masumeen-coverflow-track]';

    document.addEventListener('pointerdown', function (e) {
        const track = e.target.closest(TRACK_SEL);
        if (!track) return;
        if (e.target.closest('button, a')) return; // don't hijack a button press
        dragState = {
            startX: e.clientX, startY: e.clientY, moved: false, pointerId: e.pointerId,
            isM: track.hasAttribute('data-masumeen-coverflow-track')
        };
    });
    ...
    document.addEventListener('pointermove', function (e) {
        ...
        if (!dragState.moved && Math.abs(dx) > 10 && Math.abs(dx) > Math.abs(dy)) dragState.moved = true;
        if (dragState.moved && e.cancelable) e.preventDefault(); // stop the page scrolling sideways mid-swipe
    });
    ...
    function endDrag(e) {
        ...
        if (dragState.moved) {
            ...
            if (Math.abs(dx) >= SWIPE_THRESHOLD) {
                const nav = dragState.isM ? masumeenCoverflowNav : imamCoverflowNav;
                nav(dx < 0 ? 1 : -1);
            }
        }
        dragState = null;
    ...
    // Capture-phase: swallow the one click a drag generates, before it can
    // reach either the coverflow click handler above or script-2-ui.js's
    // data-action dispatcher.
    document.addEventListener('click', function (e) {
        if (!suppressNextClick) return;
        suppressNextClick = false;
        if (e.target.closest('[data-imam-coverflow], [data-masumeen-coverflow]')) { e.stopPropagation(); e.preventDefault(); }
    }, true);
})();
    ...
    // Imam 3D coverflow carousel — exported the same way, so
    // script-3-pages.js (a separate, non-wrapped script) can reach them.
    if (typeof g._imamCoverflowOffset !== 'function') g._imamCoverflowOffset = _imamCoverflowOffset;
    if (typeof g._imamCoverflowTier !== 'function') g._imamCoverflowTier = _imamCoverflowTier;
    if (typeof g._imamCoverflowClamp !== 'function') g._imamCoverflowClamp = _imamCoverflowClamp;
    if (typeof g.imamCoverflowApply !== 'function') g.imamCoverflowApply = imamCoverflowApply;
    if (typeof g.imamCoverflowGoTo !== 'function') g.imamCoverflowGoTo = imamCoverflowGoTo;
    if (typeof g.imamCoverflowNav !== 'function') g.imamCoverflowNav = imamCoverflowNav;
    if (typeof g.masumeenCoverflowApply !== 'function') g.masumeenCoverflowApply = masumeenCoverflowApply;
    if (typeof g.masumeenCoverflowGoTo !== 'function') g.masumeenCoverflowGoTo = masumeenCoverflowGoTo;
    if (typeof g.masumeenCoverflowNav !== 'function') g.masumeenCoverflowNav = masumeenCoverflowNav;
