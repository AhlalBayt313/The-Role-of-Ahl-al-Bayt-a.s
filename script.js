...
function renderHomePage()
{
    ...
    <div class="${d?'hero-luxury':'hero-luxury-light'} py-16 md:py-24 px-6 rounded-3xl mb-12 relative page-enter" style="min-height:380px;display:flex;align-items:center;justify-content:center;color:${d?'white':'inherit'}">
        <div class="hero-geo-bg"></div>
        <div class="islamic-geo-overlay"></div>
        <div class="hero-orb hero-orb-1"></div>
        <div class="hero-orb hero-orb-2"></div>
        ${Array.from({length:10},(_,i)=>`<div class="hero-particle" style="width:${4+i%3*2}px;height:${4+i%3*2}px;left:${8+i*9}%;bottom:-8px;background:${i%3===0?'rgba(180,83,9,.8)':i%3===1?'rgba(5,150,105,.6)':'rgba(255,255,255,.4)'};animation-duration:${8+i*1.3}s;animation-delay:${i*.7}s;--drift:${(i%2===0?1:-1)*30}px"></div>`).join('')}
        <div style="position:relative;z-index:2;text-align:center;max-width:680px;margin:0 auto">
            <div style="display:inline-block;padding:10px 24px;border-radius:50px;background:rgba(255,255,255,.09);border:1px solid rgba(255,255,255,.18);backdrop-filter:blur(8px);margin-bottom:1.5rem">
                <p class="arabic-text" dir="rtl" style="font-size:1.5rem;margin:0;text-shadow:0 0 30px rgba(180,83,9,.5)">بِسْمِ اللَّهِ الرَّحْمَٰنِ الرَّحِيمِ</p>
            </div>
            <h2 style="font-size:clamp(2rem,6vw,3.2rem);font-weight:900;line-height:1.1;margin-bottom:.6rem;text-shadow:0 4px 20px rgba(0,0,0,.25);color:${d?'#fff':'#022c22'}">${l==='bn'?'আহলে বাইত (আ.)':'Ahl al-Bayt (a.s)'}</h2>
            <p style="font-size:clamp(.95rem,2.5vw,1.15rem);opacity:.8;margin-bottom:2.25rem;line-height:1.65;color:${d?'rgba(255,255,255,.85)':'rgba(2,44,34,.8)'}">${l==='bn'?'কুরআন, হাদিস ও পবিত্র ইমামদের শিক্ষায় আলোকিত হোন':'Enlighten yourself with Quran, Hadith & Holy Imams\' teachings'}</p>
            <div style="display:flex;gap:10px;justify-content:center;flex-wrap:wrap">
                <button data-action="changePage" data-param="dua" class="btn-primary" style="background:linear-gradient(135deg,#b45309,#92400e);color:white;padding:13px 28px;border-radius:50px;font-weight:700;border:none;cursor:pointer;box-shadow:0 8px 24px rgba(180,83,9,.45);display:flex;align-items:center;gap:7px;font-size:.95rem">📿 ${l==='bn'?'দোয়া পড়ুন':'Read Duas'}</button>
                <button data-action="changePage" data-param="imams" class="btn-primary" style="background:rgba(255,255,255,.12);color:${d?'white':'rgba(2,44,34,.9)'};padding:13px 28px;border-radius:50px;font-weight:700;border:1.5px solid rgba(255,255,255,.3);cursor:pointer;backdrop-filter:blur(8px);display:flex;align-items:center;gap:7px;font-size:.95rem">👑 ${l==='bn'?'ইমামগণ':'The Imams'}</button>
                <button data-action="changePage" data-param="blog" class="btn-primary" style="background:rgba(5,150,105,.2);color:${d?'#6ee7b7':'#022c22'};padding:13px 28px;border-radius:50px;font-weight:700;border:1.5px solid rgba(5,150,105,.35);cursor:pointer;backdrop-filter:blur(8px);display:flex;align-items:center;gap:7px;font-size:.95rem">📖 ${l==='bn'?'ব্লগ':'Blog'}</button>
            </div>
        </div>
    </div>
    <div class="grid grid-cols-2 md:grid-cols-3 gap-4 mb-14">
        ${[['📖',l==='bn'?'ব্লগ':'Blog',l==='bn'?'ইসলামিক লেখা':'Islamic writings','blog','#059669'],['👑',l==='bn'?'১২ ইমাম':'12 Imams',l==='bn'?'পবিত্র ইমামগণ':'Holy Imams','imams','#7c3aed'],['🤲',l==='bn'?'দোয়া':'Duas',l==='bn'?'আহলে বাইতের দোয়া':'Prayers','dua','#b45309'],['📚',l==='bn'?'লাইব্রেরি':'Library',l==='bn'?'ইসলামিক বই':'Islamic books','library','#0369a1'],['📿',l==='bn'?'তাসবিহ':'Tasbeeh',l==='bn'?'ডিজিটাল তাসবিহ':'Digital tasbeeh','tasbeeh','#059669'],['🧠',l==='bn'?'কুইজ':'Quiz',l==='bn'?'জ্ঞান পরীক্ষা':'Test knowledge','quiz','#dc2626'],['☀️',l==='bn'?'৯৯ নাম':'99 Names',l==='bn'?'আসমাউল হুসনা':'Names of Allah','asmaul','#c9a227'],['🧭',l==='bn'?'কিবলা':'Qibla',l==='bn'?'কিবলার দিক':'Qibla direction','qibla','#0d9488'],['📅',l==='bn'?'ক্যালেন্ডার':'Calendar',l==='bn'?'হিজরি ক্যালেন্ডার':'Hijri Calendar','calendar','#7c3aed']].map(([icon,title,desc,page,color],fi)=>`
            <button data-action="changePage" data-param="${page}" class="feature-card-luxury ${d?'bg-gray-800 border-gray-700':'bg-white border-gray-100'} border text-left w-full p-5 reveal reveal-delay-${fi%4+1}" style="box-shadow:var(--shadow-sm)">
                <div class="feature-icon-wrap" style="background:${color}18;border:1px solid ${color}22">${icon}</div>
                <h3 class="font-bold text-sm mb-1">${title}</h3>
                <p class="text-xs ${d?'text-gray-400':'text-gray-500'} leading-relaxed">${desc}</p>
                <div class="mt-3 text-xs font-semibold flex items-center gap-1 group" style="color:${color};opacity:.85">${l==='bn'?'দেখুন':'Explore'} <svg width="11" height="11" viewBox="0 0 12 12" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" class="transition-transform group-hover:translate-x-1"><path d="M2 6h8M6 2l4 4-4 4"/></svg></div>
            </button>`).join('')}
    </div>
