...
function renderReadZiyaratPage() {
    ...
    // PHASE 3 (2026-08-22): identifier for the shared bookmark/favorite/copy/
    // share action bar — mirrors the id resolution already used by the
    // 'readZiyarat' dispatcher case (by id, else array position).
    const zUcId = z.id!=null ? z.id : (typeof ziyarats!=='undefined' ? ziyarats.indexOf(z) : 0);
    ...
    const versesHtml = hasVerses ? z.verses.map((v, i) => {
        ...
        onmouseleave="this.style.background='transparent'">
            ...
            <div style="padding:1.4rem 1.6rem;display:flex;flex-direction:column;justify-content:center;gap:.35rem;">
                ${v.tr?`<p style="
                    font-size:.82rem;line-height:1.6;font-style:italic;
                    color:${d?'#93c5fd':'#7c3aed'};margin:0;font-weight:500;opacity:.9;
                ">${sanitize(v.tr)}</p>`:''}
                <p style="
                    font-size:.95rem;line-height:1.85;
...
function renderReadAmalPage() {
    ...
    const stepRowHtml = (v, i) => {
        ...
        if (!v.ar) {
            return `
            <div class="dua-verse-row fade-in uc-step-col" style="
                padding:1.1rem 1.6rem 1.1rem calc(1.6rem + 30px);
            ...
            onmouseleave="this.style.background='transparent'">
                <span style="
                    position:absolute;top:.7rem;left:.7rem;width:22px;height:22px;border-radius:50%;
    ...
    // PHASE 3 (2026-08-22): identifier for the shared action bar — mirrors
    // readAmal()'s own id resolution (by id, else array position).
    const aUcId = a.id!=null ? a.id : (typeof amals!=='undefined' ? amals.indexOf(a) : 0);
...
function closePersonDetail() {
  ...
  // Phase 4: play the reverse fade+scale (.closing, CSS-driven) before
  // adding .hidden, instead of snapping straight to display:none. Purely
  // presentational — same end state (.hidden added), same timing for every
  // other line below, just delayed ~180ms so the animation can finish.
