...
// ============================================================================
// ZIYARAT GROUPING (multi-variant texts, e.g. Imam Reza's 8 Ziyarat)
// ============================================================================
// Some Imams have several distinct, independently-sourced Ziyarat texts
// (e.g. Imam Ali ibn Musa al-Rida has 8 — a Standalone/First Form, 2nd
// through 6th, a Wednesday-specific one, and one from Kamil al-Ziyarah).
// Rather than showing all of them as separate flat rows (reads as
// repetitive/cluttered — "Second Ziyarat", "Third Ziyarat", ... one after
// another), items sharing the same `.group` key (set in
// data/duas/ziyarat.json + metadata.json) are collapsed into ONE
// expandable card; see `toggleZiyaratGroup` (script-2-ui.js) and
// `state.expandedZiyaratGroups` (script-1-core.js) for the open/closed
// state. Items with no `.group`, or whose group has only one member in the
// current filtered set, render as a normal single row — untouched — so
// this only ever activates for genuine multi-item groups and is safe to
// tag onto any future entry without side effects elsewhere.
// Order is stable: each cluster appears at the position of its first
// member's first occurrence in `items`.
function clusterZiyaratItems(items) {
    const seenGroups = new Set();
    const clusters = [];
    items.forEach(entry => {
        const g = entry.z.group;
        if (!g) { clusters.push({type:'single', entry}); return; }
        if (seenGroups.has(g)) return; // already emitted as part of its cluster below
        seenGroups.add(g);
        const members = items.filter(e => e.z.group === g);
        if (members.length > 1) clusters.push({type:'group', key:g, members});
        else clusters.push({type:'single', entry}); // solo member — no point collapsing 1 item
    });
    return clusters;
}
...
function renderDuaPage() {
    ...
    const catFilters=[
        ...
        {key:'family',   icon:'👨‍👩‍👧', label:l==='bn'?'পরিবার':'Family',         color:'#db2777', bg:'rgba(219,39,119,.15)'},
        {key:'forgiveness',icon:'🕊️',label:l==='bn'?'ক্ষমা':'Forgiveness',  color:'#7c3aed', bg:'rgba(124,58,237,.15)'},
        {key:'ahlul-bayt',icon:'🕌', label:l==='bn'?'আহলুল বাইত':'Ahlul Bayt', color:'#b91c1c', bg:'rgba(185,28,28,.15)'},
        {key:'hajj-umrah',icon:'🕋', label:l==='bn'?'হজ্জ-উমরাহ':'Hajj & Umrah', color:'#065f46', bg:'rgba(6,95,70,.15)'},
        {key:'healing',  icon:'🩺', label:l==='bn'?'আরোগ্য':'Healing',        color:'#0d9488', bg:'rgba(13,148,136,.15)'},
        {key:'knowledge',icon:'📖', label:l==='bn'?'জ্ঞান':'Knowledge',       color:'#1d4ed8', bg:'rgba(29,78,216,.15)'},
        {key:'patience', icon:'⏳', label:l==='bn'?'ধৈর্য':'Patience',        color:'#a16207', bg:'rgba(161,98,7,.15)'},
        {key:'protection',icon:'🛡️',label:l==='bn'?'সুরক্ষা':'Protection',  color:'#4338ca', bg:'rgba(67,56,202,.15)'},
        {key:'quranic',  icon:'📗', label:l==='bn'?'কুরআনিক':'Quranic',       color:'#15803d', bg:'rgba(21,128,61,.15)'},
        {key:'rizq',     icon:'💰', label:l==='bn'?'রিযিক':'Sustenance',      color:'#ca8a04', bg:'rgba(202,138,4,.15)'},
        {key:'sahifa-sajjadiya',icon:'📜',label:l==='bn'?'সহিফা সাজ্জাদিয়া':'Sahifa Sajjadiyya', color:'#9333ea', bg:'rgba(147,51,234,.15)'},
    ...
    <div class="space-y-6 page-enter">
        ...
        <div class="space-y-2">
            ...
            filteredDuas.map((dua,i)=>{
                ...
                return `
                <div class="reveal uc-card-row" data-action="readDua" data-param="${idx}" role="button" tabindex="0"
                    style="display:flex;flex-direction:column;gap:8px;padding:11px 14px 10px;border-radius:14px;cursor:pointer;
                    background:${d?catInfo.color+'22':catInfo.bg};border:1px solid ${catInfo.color}${d?'55':'40'};transition:all .18s;box-shadow:var(--app-shadow-sm)">
        ...
        <div class="space-y-6">
            ...
            <div class="space-y-2">
                ...
                ${clusterZiyaratItems(group.items).map(cluster=>{
                    if (cluster.type==='single') {
                    const {z,idx}=cluster.entry;
                    ...
                    return `
                <div class="reveal uc-card-row" data-action="readZiyarat" data-param="${z.id||idx}" role="button" tabindex="0"
                    style="display:flex;flex-direction:column;gap:8px;padding:11px 14px 10px;border-radius:14px;cursor:pointer;
                    background:${zCatInfo.color}${d?'22':'1a'};border:1px solid ${zCatInfo.color}${d?'55':'40'};transition:all .18s;box-shadow:var(--app-shadow-sm)">
                    <div style="display:flex;align-items:center;gap:10px">
                    <div style="flex-shrink:0;width:32px;height:32px;border-radius:10px;
                    ...
                    ${ucActionBar('ziyarat', z.id||idx, d, l)}
                </div>`;
                    }
                    // MULTI-VARIANT GROUP (e.g. Imam Reza's 8 Ziyarat, sharing the
                    // same `.group` key) — one collapsed card; expands in place to
                    // list each variant as a compact sub-row that still opens the
                    // normal, UNCHANGED readZiyarat detail page — bookmarks, share,
                    // copy, and admin edit/delete all keep working exactly as
                    // before, per variant. See clusterZiyaratItems() above.
                    const first = cluster.members[0].z;
                    const zCatInfo = ziyaratCatFilters.find(f=>f.key===first.category) || ziyaratCatFilters[0];
                    const isOpen = (state.expandedZiyaratGroups||[]).includes(cluster.key);
                    const groupTitle = l==='bn' ? (first.groupTitleBn||first.titleBn) : (first.groupTitleEn||first.titleEn);
                    const countLabel = l==='bn'
                        ? `${toBengaliDigits(cluster.members.length)}টি ভিন্ন যিয়ারত — ট্যাপ করে দেখুন`
                        : `${cluster.members.length} different Ziyarat — tap to view`;
                    return `
                <div class="reveal" style="border-radius:14px;background:${zCatInfo.color}${d?'22':'1a'};border:1px solid ${zCatInfo.color}${d?'55':'40'};box-shadow:var(--app-shadow-sm)">
                    <div data-action="toggleZiyaratGroup" data-param="${cluster.key}" role="button" tabindex="0" aria-expanded="${isOpen?'true':'false'}"
                        ...
                        <div style="flex:1;min-width:0">
                            <p class="font-bold" style="font-size:13.5px;color:${d?'#f9fafb':'#111827'};white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${sanitize(groupTitle)}</p>
                            <p style="font-size:11px;font-weight:600;color:${zCatInfo.color};white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${countLabel}</p>
                        </div>
                        <svg width="14" height="14" viewBox="0 0 20 20" fill="none" stroke="${zCatInfo.color}" stroke-width="2.3" stroke-linecap="round" style="flex-shrink:0;transition:transform .2s;transform:rotate(${isOpen?180:0}deg)" aria-hidden="true"><path d="M5 7.5l5 5 5-5"/></svg>
                    </div>
                    ${isOpen?`
                    <div class="fade-in-fast" style="padding:0 10px 10px;display:flex;flex-direction:column;gap:6px">
                        ${cluster.members.map(({z,idx})=>`
                        <div class="reveal" data-action="readZiyarat" data-param="${z.id||idx}" role="button" tabindex="0"
                            style="display:flex;align-items:center;gap:8px;padding:9px 12px;border-radius:10px;cursor:pointer;
                            background:${d?'rgba(255,255,255,.05)':'rgba(255,255,255,.7)'};border:1px solid ${zCatInfo.color}${d?'33':'28'};transition:all .15s">
                            <div style="flex:1;min-width:0">
                                <p class="font-semibold" style="font-size:12.5px;color:${d?'#e5e7eb':'#1f2937'};white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${sanitize(l==='bn'?(z.variantLabelBn||z.titleBn):(z.variantLabelEn||z.titleEn))}</p>
                                ${z.source?`<p style="font-size:10.5px;color:${d?'#9ca3af':'#6b7280'};white-space:nowrap;overflow:hidden;text-overflow:ellipsis;margin-top:1px">${sanitize(z.source)}</p>`:''}
                            </div>
                            ${state.isAdmin?`
                            <div class="flex gap-1 flex-shrink-0" onclick="event.stopPropagation()">
                                <button data-action="editCustomDua" data-param="${z.id}" data-dtype="ziyarat"
                                    aria-label="${l==='bn'?'সম্পাদনা করুন':'Edit'} ${sanitize(l==='bn'?z.titleBn:z.titleEn)}"
                                    style="width:24px;height:24px;border-radius:7px;display:flex;align-items:center;justify-content:center;font-size:10px;cursor:pointer;background:${d?'rgba(59,130,246,.15)':'rgba(59,130,246,.1)'};border:1px solid rgba(59,130,246,.25)">✏️</button>
                                <button data-action="deleteCustomDua" data-param="${z.id}" data-dtype="ziyarat"
                                    aria-label="${l==='bn'?'মুছুন':'Delete'} ${sanitize(l==='bn'?z.titleBn:z.titleEn)}"
                                    style="width:24px;height:24px;border-radius:7px;display:flex;align-items:center;justify-content:center;font-size:10px;cursor:pointer;background:rgba(220,38,38,.1);border:1px solid rgba(220,38,38,.2)">🗑</button>
                            </div>`:''}
                            <svg width="11" height="11" viewBox="0 0 12 12" fill="none" stroke="${zCatInfo.color}" stroke-width="2.3" stroke-linecap="round" style="flex-shrink:0" aria-hidden="true"><path d="M2 6h8M6 2l4 4-4 4"/></svg>
                        </div>`).join('')}
                    </div>`:''}
                </div>`;
                }).join('')}
        ...
        <div class="space-y-2">
            ...
            filteredAmals.map((a,i)=>{
                ...
                return `
                <div class="reveal uc-card-row" data-action="readAmal" data-param="${a.id!=null?a.id:i}" role="button" tabindex="0"
                    style="display:flex;flex-direction:column;gap:8px;padding:11px 14px 10px;border-radius:14px;cursor:pointer;
                    background:${d?catInfo.color+'22':catInfo.bg};border:1px solid ${catInfo.color}${d?'55':'40'};transition:all .18s;box-shadow:var(--app-shadow-sm)">
                    <div style="display:flex;align-items:center;gap:10px">
                    <div style="flex-shrink:0;width:32px;height:32px;border-radius:10px;
                    ...
                    ${ucActionBar('amal', a.id!=null?a.id:i, d, l)}
                </div>`;
            }).join('')}
        </div>`:''}
...
function renderCalendarPage() {
    ...
    // Consumed once per render — only an actual calPrev/calNext click sets
    // this, so an unrelated render() while already on this page (dark mode
    // toggle, etc.) doesn't replay the slide.
    const navDir = calState.lastNavDir;
    calState.lastNavDir = null;
    const monthDirClass = navDir === 'prev' ? 'cal-month-slide-prev' : navDir === 'next' ? 'cal-month-slide-next' : '';
    ...
    return `
    <div class="space-y-6">
        <h1 class="text-3xl font-bold reveal">📅 ${t('calendar')}</h1>
        <div class="${d?'bg-gray-800 border-gray-700':'bg-white border-gray-200'} border rounded-2xl overflow-hidden reveal" style="box-shadow:0 4px 24px rgba(0,0,0,.10)" role="group" aria-label="${monthNameHijri} ${l==='bn'?toBengaliDigits(year):year} ${l==='bn'?'হিজরি ক্যালেন্ডার':'Hijri calendar'}">
            ...
            <div style="background:${d?'#1e3a2f':'#166534'}" class="px-5 py-4 flex items-center justify-between">
                ...
                <div class="text-center text-white ${monthDirClass}">
            ...
            <!-- ── CELL GRID ── -->
            <div class="grid grid-cols-7 ${monthDirClass}" style="gap:1px;background:${d?'#374151':'#e2e8f0'}">
                ${cells.map((day,ci)=>{
...
function renderContactPage() {
    ...
    return `
    <div class="space-y-8">
        <h1 class="text-3xl font-bold reveal">✉️ ${t('contact')}</h1>
        <div class="grid md:grid-cols-2 gap-8">
            <div class="${d?'bg-gray-800':'bg-white'} border rounded-xl p-8 reveal">
            ...
            <div class="space-y-6">
                <div class="${d?'bg-gray-800':'bg-white'} border rounded-xl p-6 reveal">
...
function renderImamsPage()
{
    ...
    const chips=(l==='bn'?CHIP_BN:CHIP_EN).map((name,i)=>`
        <button
            onclick="(function(){if(typeof imamCoverflowGoTo==='function'){imamCoverflowGoTo(${i});}const el=document.getElementById('imam-anchor-${i+1}');scrollToImamEl(el);})()"
        ...
        <div class="imam-flip-wrapper" style="height:100%;position:relative">
            ...
            <div class="imam-card-luxury imam-card-front imam-anim-${animVariant} border text-center p-5"
                ...
                <div class="flex gap-2">
                    ${(typeof im.id==='string' && im.id!=='p' && im.id!=='f' && !im.birthBn)
                        ? `<button onclick="ahlulBaytSelectSearchResult('extra:${im.id}')"
                    ...
                    ${(typeof im.id==='string' && im.id!=='p' && im.id!=='f')
                        ? `<button onclick="shareImamQuote((window.__nabiMasumeenExtraCards||[]).find(function(x){return x.id==='${im.id}'}),'${l}')"
    ...
    <div class="space-y-8 page-enter">
        ...
        <!-- Note: Zaynab (AS) and Abbas (AS) below are shown in this same visual
             section per explicit user request, but they are NOT among the 14
             Ma'sumeen (Prophet, Fatima, 12 Imams) in Twelver Shia belief. The
             section heading text itself is left unchanged (also per request);
             only the card list and count badge are extended. -->
        ${(function(){
            const extraSource = (typeof ADDITIONAL_PERSONALITIES!=='undefined' && Array.isArray(ADDITIONAL_PERSONALITIES)) ? ADDITIONAL_PERSONALITIES : [];
            const zainabBase = extraSource.find(p=>p.id==='zainab');
            const abbasBase  = extraSource.find(p=>p.id==='abbas');
            const extras = [
                zainabBase ? Object.assign({}, zainabBase, {
                    quoteBn: 'আমি সৌন্দর্য ছাড়া আর কিছুই দেখিনি।',
                    quoteEn: 'I have seen nothing but beauty.'
                }) : null,
                abbasBase ? Object.assign({}, abbasBase, {
                    quoteBn: 'আল্লাহ আব্বাসের প্রতি রহম করুন — তিনি ত্যাগ স্বীকার করেছেন এবং নিজের ভাইয়ের জন্য জীবন উৎসর্গ করেছেন।',
                    quoteEn: 'May Allah have mercy on Abbas — he strove and gave his life for his brother.'
                }) : null
            ].filter(Boolean);
            window.__nabiMasumeenExtraCards = extras;
            const nabiMasumeenList = masumeen.concat(extras);

            const mCount  = nabiMasumeenList.length;
            const mActive = mCount ? _imamCoverflowClamp(_masumeenCoverflowIndex, mCount) : 0;
            const masumeenCards = nabiMasumeenList.map((im,idx)=>{
                const offset = _imamCoverflowOffset(idx, mActive, mCount);
                const tier   = _imamCoverflowTier(offset);
                return `
                <div class="imam-coverflow-card ${tier}"
                    data-masumeen-coverflow-card data-masumeen-index="${idx}"
                    id="masumeen-anchor-${im.id}"
                    style="--cf-offset:${offset}"
                    aria-current="${offset===0?'true':'false'}"
                    aria-label="${sanitize(l==='bn'?im.nameBn:im.nameEn)}"
                    tabindex="${offset===0?'0':'-1'}">
                    <div style="min-height:320px">${renderCard(im,idx,MACS,MACS2,MCONIC,idx)}</div>
                </div>`;
            }).join('');
            const masumeenDots = nabiMasumeenList.map((im,idx)=>`
                <button type="button" class="imam-coverflow-dot${idx===mActive?' is-active':''}"
                    data-masumeen-coverflow-dot="${idx}"
                    aria-current="${idx===mActive?'true':'false'}"
                    aria-label="${l==='bn'?(idx+1)+' নম্বরে যান':'Go to '+(idx+1)}"></button>`).join('');

            return `
        <div class="reveal imam-coverflow-section" data-masumeen-coverflow-section>
            <div style="background:${d?'linear-gradient(135deg,rgba(201,162,39,.1),rgba(180,83,9,.04))':'linear-gradient(135deg,rgba(201,162,39,.07),rgba(180,83,9,.02))'};
                border:1.5px solid ${d?'rgba(201,162,39,.3)':'rgba(201,162,39,.28)'};
                border-radius:var(--r-lg);padding:1rem 1.25rem;margin-bottom:1.75rem;
                box-shadow:${d?'none':'0 3px 12px rgba(201,162,39,.1)'}">
                <div class="section-heading" style="margin-bottom:0">
                    ...
                    <span class="text-xs font-bold px-2.5 py-1 rounded-full"
                        ...
                        ${l==='bn'?toBengaliDigits(mCount)+' জন':mCount}
                    </span>
                </div>
            </div>
            <div class="imam-coverflow-nav-wrap">
                <button type="button" class="imam-coverflow-nav-btn" data-masumeen-coverflow-nav="-1"
                    aria-label="${l==='bn'?'পূর্ববর্তী':'Previous'}">
                ...
                <div class="imam-coverflow" data-masumeen-coverflow tabindex="0"
                    role="region" aria-roledescription="carousel"
                    aria-label="${l==='bn'?'নবী ও মাসুমিনের কার্ড ক্যারোজেল':'Prophet & Masumeen carousel'}">
                    <div class="imam-coverflow-track" data-masumeen-coverflow-track>
                        ${masumeenCards}
                    </div>
                </div>
                <button type="button" class="imam-coverflow-nav-btn" data-masumeen-coverflow-nav="1"
                    aria-label="${l==='bn'?'পরবর্তী':'Next'}">
            ...
            <div class="imam-coverflow-dots" data-masumeen-coverflow-dots>${masumeenDots}</div>
        </div>`;
        })()}

        <!-- ── ১২ ইমাম section: 3D coverflow carousel ── -->
        ${(function(){
            const cfCount  = imams.length;
            const cfActive = cfCount ? _imamCoverflowClamp(_imamCoverflowIndex, cfCount) : 0;
            const coverflowCards = imams.map((im,idx)=>{
                const offset = _imamCoverflowOffset(idx, cfActive, cfCount);
                const tier   = _imamCoverflowTier(offset);
                return `
                <div class="imam-coverflow-card ${tier}"
                    data-coverflow-card data-imam-index="${idx}" data-imam-id="${im.id}"
                    id="imam-anchor-${im.id}"
                    style="--cf-offset:${offset}"
                    aria-current="${offset===0?'true':'false'}"
                    aria-label="${sanitize(l==='bn'?im.nameBn:im.nameEn)}"
                    tabindex="${offset===0?'0':'-1'}">
                    ${renderCard(im,idx,ACS,ACS2,CONIC,idx+masumeen.length)}
                </div>`;
            }).join('');
            const coverflowDots = imams.map((im,idx)=>`
                <button type="button" class="imam-coverflow-dot${idx===cfActive?' is-active':''}"
                    data-imam-coverflow-dot="${idx}"
                    aria-current="${idx===cfActive?'true':'false'}"
                    aria-label="${l==='bn'?(idx+1)+' নম্বর ইমামে যান':'Go to Imam '+(idx+1)}"></button>`).join('');
            return `
        <div class="reveal imam-coverflow-section" data-imam-coverflow-section>
            ...
            <div class="imam-coverflow-nav-wrap">
                <button type="button" class="imam-coverflow-nav-btn" data-imam-coverflow-nav="-1"
                    aria-label="${l==='bn'?'পূর্ববর্তী ইমাম':'Previous Imam'}">
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M15 18l-6-6 6-6"/></svg>
                </button>
                <div class="imam-coverflow" data-imam-coverflow tabindex="0"
                    role="region" aria-roledescription="carousel"
                    aria-label="${l==='bn'?'বারো ইমামের কার্ড ক্যারোজেল':'The Twelve Imams carousel'}">
                    <div class="imam-coverflow-track" data-imam-coverflow-track>
                        ${coverflowCards}
                    </div>
                </div>
                <button type="button" class="imam-coverflow-nav-btn" data-imam-coverflow-nav="1"
                    aria-label="${l==='bn'?'পরবর্তী ইমাম':'Next Imam'}">
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M9 18l6-6-6-6"/></svg>
                </button>
            </div>
            <div class="imam-coverflow-dots" data-imam-coverflow-dots>${coverflowDots}</div>
        </div>`;
        })()}
...
function renderImamDetailPage()
{
    ...
    // Extra personalities (string ids like 'zainab' that now carry full
    // birthBn/fullBioBn data, routed here via viewImam's new fallback) are
    // not part of masumeen/imams, so imams.indexOf(im) below would return
    // -1 for them — handled via isExtra using the same MACS gold/pink
    // palette the card grid already colors them with, keyed off their
    // position in the extras cache instead.
    const isExtra=!isMasumeen && typeof im.id==='string';
    const colorIdx=isMasumeen?masumeen.findIndex(m=>m.id===im.id)
        :isExtra?(window.__nabiMasumeenExtraCards||[]).findIndex(x=>x.id===im.id)
        :imams.indexOf(im);
    ...
    const ac=(isMasumeen||isExtra)?MACS[((colorIdx%2)+2)%2]:ACS[colorIdx%12];
    const ac2=(isMasumeen||isExtra)?MACS2[((colorIdx%2)+2)%2]:ACS2[colorIdx%12];
    const conic2=(isMasumeen||isExtra)?MCONIC2[((colorIdx%2)+2)%2]:CONIC2[colorIdx%12];
    return `
    <div class="max-w-2xl mx-auto page-enter">
        ...
        <div class="card-luxury ${d?'bg-gray-800 border-gray-700':'bg-white border-gray-100'} border mb-6"
            ...
            <div class="p-6 pt-2">
                ...
                ${(l==='bn' && Array.isArray(im.fullBioBn) && im.fullBioBn.length) ? `
                ...
                ${(l==='bn' && Array.isArray(im.sourcesBn) && im.sourcesBn.length) ? `
                <div class="rounded-2xl p-5 mt-4" style="background:${d?'rgba(255,255,255,.03)':'rgba(0,0,0,.02)'};border:1px solid ${d?'rgba(255,255,255,.08)':'rgba(0,0,0,.06)'}">
                    <h3 class="font-bold mb-2 text-xs" style="color:${d?'#9ca3af':'#6b7280'}">📚 তথ্যসূত্র</h3>
                    <ol style="font-size:.72rem;line-height:1.85;color:${d?'#9ca3af':'#6b7280'};padding-inline-start:1.1rem;margin:0">
                        ${im.sourcesBn.map(s => `<li>${sanitize(s)}</li>`).join('')}
                    </ol>
                </div>` : ''}
            </div>
        </div>
...
function renderQuizSetupPage() {
    ...
    return `
    <div class="space-y-8">
        <div class="flex flex-wrap justify-between items-center gap-4 reveal">
        ...
        <button data-action="quizStartDaily" class="w-full text-left rounded-2xl transition-all hover:scale-[1.01] reveal" style="background:linear-gradient(135deg,#7f1d1d,#dc2626);padding:1.4rem 1.6rem;box-shadow:0 8px 28px rgba(220,38,38,.3)">
...
function renderTasbeehPage()
{
    ...
    <div class="space-y-6 page-enter">
        ...
        <!-- Dhikr selector -->
        <div style="overflow-x:auto;-webkit-overflow-scrolling:touch;scrollbar-width:none;padding:2px 1px 6px" class="reveal hscroll-fade">
...
function renderAboutPage() {
    ...
    return `
    <div class="space-y-8">
        <h1 class="text-3xl font-bold reveal">ℹ️ ${t('about')}</h1>
        <article class="${d?'bg-gray-800':'bg-white'} border rounded-xl p-8 reveal">
...
function renderBookmarksPage() {
    ...
    return `
    <div class="space-y-6">
        <h2 class="text-3xl font-bold reveal">🔖 ${t('bookmarks')}</h2>
        <div class="flex gap-2 flex-wrap">
...
function renderAnalyticsPage() {
    ...
    return `
    <div class="space-y-8">
        <h2 class="text-3xl font-bold reveal">📊 ${t('analytics')}</h2>
        ...
        <div class="${d?'bg-gray-800':'bg-white'} border rounded-2xl p-6 reveal">
