...
// ============================================================================
// OVERLAY CLOSE ANIMATION HELPER
// ============================================================================
// Mirrors closePersonDetail()'s pattern (script-4-boot.js): add a .closing
// class (CSS in style.css plays the reverse fade+scale), then delay the
// actual state flip + render() (which unmounts the overlay) until the
// animation finishes, instead of snapping straight to removal. Shared by
// all 9 editor/login overlay close cases below since only one such overlay
// is ever open at a time, so one generic selector finds whichever is open.
// The `applyStateChange` callback is exactly the state-mutation line each
// case already had — nothing about what each close does is changed, only
// when render() fires afterward.
//
// Both this function and closePersonDetail() used to each hardcode their
// own copy of the "check prefers-reduced-motion, add .closing, delay by
// 180ms/0ms" timing logic. That's now factored into _animatedClose() below
// (shared by both) — only the duplicated timing mechanics moved; what each
// caller does once the delay is over (state+render here, hidden+focus in
// closePersonDetail) is unchanged.
function _animatedClose(el, onComplete) {
    const reduceMotion = window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    el.classList.add('closing');
    setTimeout(onComplete, reduceMotion ? 0 : 180);
}

function closeOverlayAnimated(applyStateChange) {
    const el = document.querySelector(
        '.modal-overlay, #knowledge-editor-overlay, #dua-editor-overlay, .fixed.inset-0.z-50:has(> .fade-in)'
    );
    if (!el || el.classList.contains('closing')) {
        // Already animating out (e.g. a second click during the delay), or
        // not found (defensive) — just apply the state change directly.
        applyStateChange();
        render();
        return;
    }
    _animatedClose(el, () => { applyStateChange(); render(); });
}
...
function setupEventListeners() {
    ...
    document.addEventListener('click', e => {
        ...
        if (btn) {
            ...
            switch(action){
                ...
                case 'toggleZiyaratGroup': {
                    // NEW: expand/collapse a multi-variant Ziyarat group (e.g. Imam
                    // Reza's 8 texts, sharing the same `.group` key) in the list —
                    // mirrors toggleMuharramEvent's exact array-toggle idiom above.
                    const arr = state.expandedZiyaratGroups || (state.expandedZiyaratGroups=[]);
                    const idx = arr.indexOf(param);
                    if (idx > -1) arr.splice(idx,1); else arr.push(param);
                    render(); break;
                }
                ...
                // Extra-personality fallback added alongside viewImam's below (Zaynab/
                // Abbas' new detail page's own Share button routes through here by
                // im.id — without this it silently did nothing for them).
                case 'shareImamQuote': { const im2=imams.find(x=>x.id===parseInt(param))||masumeen.find(x=>x.id===param)
                    ||(window.__nabiMasumeenExtraCards||[]).find(x=>x.id===param)
                    ||(typeof ADDITIONAL_PERSONALITIES!=='undefined'?ADDITIONAL_PERSONALITIES.find(x=>x.id===param):null);
                    if(im2) shareImamQuote(im2,state.language); break; }
                case 'toggleBookmark': toggleBookmark(param,param2); break;
                // PHASE 3 (2026-08-22): shared Copy/Share/Favorite actions for
                // Dua/Ziyarat/Amal cards & detail pages — see ucActionBar()
                // and its helper functions in script-1-core.js.
                case 'ucCopy': if(typeof ucCopyItem==='function') ucCopyItem(param,param2); break;
                case 'ucShare': if(typeof ucShareItem==='function') ucShareItem(param,param2); break;
                case 'ucToggleFavorite': if(typeof toggleKcFavorite==='function') toggleKcFavorite(param,param2); break;
                ...
                case 'calPrev': calState.lastNavDir='prev'; calState.hijriMonth--; if(calState.hijriMonth<1){calState.hijriMonth=12;calState.hijriYear--;} render(); break;
                case 'calNext': calState.lastNavDir='next'; calState.hijriMonth++; if(calState.hijriMonth>12){calState.hijriMonth=1;calState.hijriYear++;} render(); break;
                case 'showAdminLogin': state.showAdminLogin=true; render(); break;
                case 'closeAdminLogin': closeOverlayAnimated(() => { state.showAdminLogin=false; state.adminLoginError=''; }); break;
                ...
                case 'closeQuizEditor': closeOverlayAnimated(() => { state.showQuizEditor=false; state.editingQuizQuestion=null; }); break;
                ...
                // PHASE 3 sub-phase B (2026-08-22): inline accordion expand/
                // collapse for Masail/Fatwa/Q&A cards — same array-of-keys
                // pattern as the existing 'toggleMuharramEvent' case above.
                // Purely additive to the existing "বিস্তারিত/View" full
                // detail-page navigation (kcOpenDetail, untouched).
                case 'toggleKcAccordion': {
                    const arr = state.kcExpanded || (state.kcExpanded=[]);
                    const key = `${param}-${param2}`;
            ...
            closeOverlayAnimated(() => { state.showKnowledgeEditor=false;state.editingKnowledgeItem=null; }); break;
        case 'openDuaEditor': openDuaEditor(null, param); break;
                ...
                case 'closeHadithEditor': closeOverlayAnimated(() => { state.showHadithEditor=false; state.editingHadith=null; }); break;
                case 'editHadith': {
                    const idx=parseInt(param);
                ...
                case 'closeAyahEditor': closeOverlayAnimated(() => { state.showAyahEditor=false; state.editingAyah=null; }); break;
                case 'editAyah': {
                    const idx=parseInt(param);
                ...
                // IMAM PAGE
                case 'viewImam': {
                    // Extra fallback (added for Zaynab bint Ali's full detail page): extra
                    // personalities with string ids (e.g. 'zainab') that now carry full
                    // birthBn/martyrdomBn/fullBioBn/sourcesBn data are looked up from the
                    // nabiMasumeenList extras cache the card grid builds
                    // (window.__nabiMasumeenExtraCards, set in renderImamsPage()) first,
                    // then — bug fix, 2026-09-03 — ADDITIONAL_PERSONALITIES directly as a
                    // final fallback, since the cache is only populated after that page has
                    // rendered at least once; reaching an extra via search or the Male/
                    // Female Personalities tabs without ever visiting /imams first would
                    // otherwise silently fail. imams/masumeen lookups above are untouched.
                    state.currentImam=imams.find(im=>im.id===parseInt(param)) || masumeen.find(m=>m.id===param)
                        || (window.__nabiMasumeenExtraCards||[]).find(x=>x.id===param)
                        || (typeof ADDITIONAL_PERSONALITIES!=='undefined' ? ADDITIONAL_PERSONALITIES.find(p=>p.id===param) : null);
                ...
                case 'closeMuharramEditor': { closeOverlayAnimated(() => { state.showMuharramEditor=false; state.editingMuharramEvent=null; }); break; }
                ...
                case 'closeShiaDayEditor': { closeOverlayAnimated(() => { state.showShiaDayEditor=false; state.editingShiaDay=null; }); break; }
                case 'saveShiaDay': {
    ...
    // Keyboard activation for non-native [data-action] controls (e.g. the
    // Dua/Ziyarat/Amal uc-card-row divs, which carry role="button"
    // tabindex="0" but aren't real <button>/<a> elements, so browsers don't
    // fire click on Enter/Space for them automatically). Delegated, additive
    // — the existing click listener above is untouched; this just simulates
    // a click for keyboard users on the same [data-action] elements.
...
function renderHeader()
{
    ...
    <header style="background:${bg};border-bottom:1.5px solid ${border};" class="sticky top-0 z-30" id="main-header">
        <div class="max-w-7xl mx-auto px-4" style="padding-top:14px;padding-bottom:14px">
            <div class="flex items-center justify-between gap-2">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <button data-action="toggleMenu" aria-label="${t('menu')}"
                        ...
                        style="width:44px;height:44px;border-radius:12px;display:flex;align-items:center;justify-content:center;background:${d?'rgba(255,255,255,.07)':'rgba(0,0,0,.05)'}">
                ...
                <div>
                    <div class="flex items-center gap-3 mb-5">
                        <div style="display:flex;align-items:center;flex-shrink:0">
                            <img src="assets/images/logo-mark.png" alt="${l==='bn'?'আহলে বাইত (আ.)':'Ahl al-Bayt (a.s)'}" loading="lazy" class="lazy-fade" style="height:46px;width:46px;border-radius:50%;display:block;box-shadow:0 4px 18px rgba(0,0,0,.25)" />
                        </div>
                        <div>
...
function renderHomePage()
{
    ...
    <div class="${d?'hero-luxury':'hero-luxury-light'} rounded-3xl mb-12 relative page-enter overflow-hidden"
        ...
        <!-- Geo grid (Step 1 — Hero Pattern Refinement: merged the redundant
             islamic-geo-overlay crosshatch into hero-geo-bg below; both were
             near-identical diagonal line grids stacked on top of each other) -->
        ...
        <div style="position:relative;z-index:2;text-align:center;max-width:700px;margin:0 auto">
            ...
            <div class="hero-bismillah-pill">
                ...
                <p class="arabic-text" dir="rtl" style="font-size:clamp(1rem,4.5vw,1.35rem);margin:0;color:${d?'#f59e0b':'#78350f'}">بِسْمِ اللَّهِ الرَّحْمَٰنِ الرَّحِيمِ</p>
            ...
            <!-- Main heading -->
            <h1 style="font-size:clamp(2rem,6vw,3.4rem);font-weight:900;line-height:1.15;margin-bottom:.65rem;
            ...
            <!-- Tagline -->
            <!-- Step 5 — Final Polish: light-mode alpha was .58, measuring ~3.9:1
                 against this hero's pale gradient — under the 4.5:1 AA floor for
                 this text size. Bumped to .70 (same #022c22 green, no new color)
                 for ~5.3:1. Dark-mode alpha (.78, already high contrast) unchanged. -->
            <p style="font-size:clamp(.88rem,2.5vw,1.05rem);line-height:1.75;margin-bottom:2rem;
                color:${d?'rgba(255,255,255,.78)':'rgba(2,44,34,.70)'};
            ...
            <div style="display:flex;gap:12px;justify-content:center;flex-wrap:wrap;margin-bottom:8px">
                <button data-action="changePage" data-param="imams" class="btn-primary hero-cta-btn"
                    ...
                    color:${d?'white':'#022c22'};
                    border:1.5px solid ${d?'rgba(52,211,153,.35)':'rgba(5,150,105,.28)'};
                    box-shadow:${d?'0 4px 20px rgba(5,150,105,.3)':'0 4px 20px rgba(5,150,105,.12)'}">
                    👑 ${l==='bn'?'ইমামগণ':'The Imams'}
                </button>
                <button data-action="changePage" data-param="dua" class="btn-primary hero-cta-btn"
                    style="background:${d?'rgba(124,58,237,.2)':'rgba(124,58,237,.1)'};
                    color:${d?'#c4b5fd':'#5b21b6'};
                    border:1.5px solid ${d?'rgba(124,58,237,.35)':'rgba(124,58,237,.25)'}">
                    🤲 ${l==='bn'?'দোয়া':'Duas'}
                </button>
                <button data-action="changePage" data-param="blog" class="btn-primary hero-cta-btn"
                    style="background:${d?'rgba(3,105,161,.22)':'rgba(3,105,161,.1)'};
                    color:${d?'#7dd3fc':'#075985'};
                    border:1.5px solid ${d?'rgba(3,105,161,.35)':'rgba(3,105,161,.25)'}">
                    📝 ${t('blog')}
                </button>
            </div>
